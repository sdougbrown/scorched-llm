...
import type { PlayerSpec } from '../src/config/schema.js'
...
import { createFableAgent } from '../src/match/fable-agent.js'
...
async function runDuel(seed: number, opponent: OpponentKind, fableSeat: 0 | 1) {
  ...
  const agents = names.map((name, i) =>
    name === 'fable' ? createFableAgent(`tank-${i}`, config) : makeOpponent(opponent, `tank-${i}`),
  )
...
describe('fable agent', () => {
  ...
  it('passes when it is not its turn', async () => {
    const agent = createFableAgent('tank-0')
  ...
  it('works without a config (falls back to duel defaults)', async () => {
    const players: PlayerSpec[] = [
      { label: 'fable', startPosition: 'random', scripted: 'fable' },
      { label: 'aggressive', startPosition: 'random', scripted: 'aggressive' },
    ]
    const config = PRESETS.duel(42, players)
    const agents = [createFableAgent('tank-0'), createAggressiveAgent('tank-1')]
  ...
  it('holds its own in the survival preset', async () => {
    ...
    const agents = names.map((name, i) =>
      name === 'fable'
        ? createFableAgent(`tank-${i}`, config)
